...
export class DragScrollComponent implements OnDestroy, AfterViewInit, OnChanges, AfterViewChecked {
   ...
   */
  moveLeft() {
    // console.log("THIS METHOD move Left WAS CALLED " + (this.$_COUNT_CALL++));
  ...
  $_COUNT_CALL = 1;
  moveRight() {
    // console.log("THIS METHOD move Right WAS CALLED " + (this.$_COUNT_CALL++));
    ...
    if (!this.isScrollReachesRightEnd() && this.currIndex < this.maximumIndex(containerWidth, this._children.toArray())) {
      this.currIndex++;
      clearTimeout(this.scrollToTimer as number);
      // console.log("snap duration: " + this.snapDuration);
      // console.log("this._contentRef.nativeElement: " + this._contentRef.nativeElement);
      // console.log("this.toChildrenLocation(): " + this.toChildrenLocation());
  ...
  */
  ...
  private scrollTo(element: Element, to: number, duration: number) {
    ...
    // const start = element.scrollLeft, to - 75.375
    
    if (this.$_FIRST_TO_FLAG) {
      this.$_FIRST_TO = to
      this.$_FIRST_TO_FLAG = false
    }

    to *= 7
    let start = element.scrollLeft

    if (start == 0) this.$_BACK_FLAG = false

    if (this.$_BACK_FLAG) to -= this.$_FIRST_TO * 5

    if (start + this.$_FIRST_TO * 7 > (this._children.toArray().length - 1) * this.$_FIRST_TO + 1) {
      to = start - this.$_FIRST_TO * 7
      console.log("here");
      this.$_BACK_FLAG = true
    }
    
    let change = to - start - this.snapOffset,
      increment = 20;
    let currentTime = 0;
    ...
    var el = <HTMLElement>document.getElementById("dc-scroll")
    console.log("el: " + el.getAttribute("drag-scroll-x-disabled"));

    const container = this.wrapper || this.parentNode;
    const containerWidth = container ? container.clientWidth : 0;
    console.log("this.maximumIndex(containerWidth, this._children.toArray()): " + this.maximumIndex(containerWidth, this._children.toArray()));
    ...
    const animateScroll = function () {
      ...
      // console.log("start to scroll: " + start);
      // console.log("change to scroll: " + change);
  ...
  private locateCurrentIndex(snap?: boolean) {
    console.log("LOCATE CURRENT INDEX");
  ...
  private toChildrenLocation(): number {
    ...
    for (let i = 0; i < this.currIndex; i++) {
      ...
      // console.log("childrenArr[i]._elementRef.nativeElement.clientWidth: " + childrenArr[i]._elementRef.nativeElement.parentNode.getBoundingClientRect().width);
      // to += childrenArr[i]._elementRef.nativeElement.clientWidth;
      to += childrenArr[i]._elementRef.nativeElement.parentNode.getBoundingClientRect().width;
    }
    return to;
  }
  ...
  private isScrollReachesRightEnd(): boolean {
    // console.log("this._contentRef.nativeElement.scrollLeft = " + this._contentRef.nativeElement.scrollLeft);
    // console.log("this._contentRef.nativeElement.offsetWidth = " + this._contentRef.nativeElement.offsetWidth);
    ...
    // console.log("this._contentRef.nativeElement.scrollWidth = " + this._contentRef.nativeElement.scrollWidth);
    
    // console.log("scrollLeftPos >= this._contentRef.nativeElement.scrollWidth = " + (scrollLeftPos >= this._contentRef.nativeElement.scrollWidth));
    
    

    return Math.ceil(scrollLeftPos) >= this._contentRef.nativeElement.scrollWidth;
  }
